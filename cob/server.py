...
binOpts = {
    ...
    "hpo": {"default": dflt["hpo"], "state": dflt["hpo"], "isBool": True},
    "visEnrich": {
...
js_files = [
    ...
    "lib/jquery.textcomplete-1.8.1.min.js",
    "lib/bootstrap-3.3.7.min.js",
    "lib/datatables-1.10.18.min.js",
    "lib/qtip-3.0.3.min.js",
    "lib/download-1.4.5.min.js",
    "lib/cytoscape-3.4.0.min.js",
    "lib/cytoscape-qtip-2.7.1.js",
    "lib/cytoscape-graphml-1.0.5.js",
    "lib/filepond.js",
    # load cob libraries
    "core.js",
    "genes.js",
...
css_files = [
    "lib/bootstrap-3.3.7.min.css",
    "lib/datatables-1.10.18.min.css",
    "lib/qtip-3.0.3.min.css",
    "lib/filepond.css",
    "cob.css",
]
...
def bundle_files(files, type, static_bundle_dir=static_bundle_dir):
    ...
    with open(os.path.join(static_bundle_dir, type, "bundle." + type), "w") as bundle:
        for fn in files:
...
def available_datasets(type=None, *args):
    ...
    return jsonify(
        {"data": list(datasets[["Name", "Description"]].itertuples(index=False))}
    )
...
def term_network_stats():
    ...
    def df_to_list(df):
        ...
        for k, v in df.iterrows():
            str_rep = (
                v["gene_a"] + " " + v["gene_b"] + "{'score':" + str(v["score"]) + "}"
            )
    ...
    node_conn = pd.DataFrame(
        pd.Series(
            nx.average_node_connectivity(edge_graph), index=["avg_node_connectivity"]
        )
    )
    print(node_conn)
    # get top ten
    Sub_Net_stats = pd.DataFrame(
        [i[1] for i in sorted_x[-10:]], index=[j[0] for j in sorted_x[-10:]]
    )
...
def term_network():
    ...
    if fdrCutoff and ontology.name in gwas_data_db and not (hpo):
        ...
        gwas_data = gwas_data[
            gwas_data["SNP2Gene"] == ("strongest" if strongestSNPs else "effective")
        ]
        gwas_data = gwas_data[
            gwas_data["Method"] == ("density" if overlapDensity else "locality")
        ]
        net["nodes"] = getNodes(
            genes,
            cob,
...
def gen_nx():
    return term_stats()
    # call term_network get json
    # uo = nx.readwrite.json_graph(term_network())
    # return jsonify({"lisa":"orange"})#term_network()
...
def custom_network():
    ...
    for name in copy.copy(rejected):
        ...
        if visNeighbors is not None:
            ...
            nbs = cob.neighbors(
                gene, names_as_index=False, names_as_cols=True
            ).sort_values("score")
...
def gene_connections():
    ...
    if len(newGenes) > 0:
        edges = list(
            filter(
                lambda x: (
                    (x["data"]["source"] in newGenes)
                    or (x["data"]["target"] in newGenes)
                ),
                edges,
            )
        )
...
def go_enrichment():
    ...
    enr = gont.enrichment(
        genes, pval_cutoff=pCutoff, min_term_size=minTerm, max_term_size=maxTerm
    )
    if len(enr) == 0:
        abort(400)
...
def getEdges(geneList, cob):
    ...
    subnet = cob.subnetwork(
        cob.refgen.from_ids(geneList), names_as_index=False, names_as_cols=True
    )
    ...
    edges = [
        {
            "group": "edges",
            "data": {"source": source, "target": target, "weight": str(weight)},
        }
